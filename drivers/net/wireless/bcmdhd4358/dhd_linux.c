 ...
 *
 * $Id: dhd_linux.c 485800 2014-06-17 15:04:50Z $
 */
...
#if defined(BCMPCIE) || (defined(BCMLXSDMMC) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)))
	if (ifidx == 0) {
#ifdef BCMLXSDMMC
	...
	if (dhd->dhd_state & DHD_ATTACH_STATE_PROT_ATTACH) {
		...
		if (is_reboot == SYS_RESTART) { 
			...
			if (dhd_wifi_platdata && !dhdp->dongle_reset) {
				...
				wifi_platform_set_power(dhd_wifi_platdata->adapters, FALSE,WIFI_TURNOFF_DELAY);
			}
		}
#endif
		...
		if(!flag) {
			if(dhd->pub.busstate == DHD_BUS_DOWN) {
				...
				while(retry--) {
					...
					if(!ret) {
						printk("BRCM_CH: dhdpcie_bus_clock_start OK\n");
						break;
					}
					else
						OSL_SLEEP(10);
