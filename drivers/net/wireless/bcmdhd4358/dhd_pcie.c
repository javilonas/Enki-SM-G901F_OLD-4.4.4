 ...
 *
 * $Id: dhd_pcie.c 485576 2014-06-16 12:56:37Z $
 */
...
dhdpcie_bus_suspend(struct  dhd_bus *bus, bool state)
{
	...
	if (state) {
		...
		if (bus->wait_for_d3_ack) {
			...
			DHD_ERROR(("%s: D3 Ack timeout\n", __FUNCTION__));
			return -ETIMEDOUT;
		}
		bus->wait_for_d3_ack = 1;
		DHD_ERROR(("%s: Suspend completed\n", __FUNCTION__));
	}
	else {
		/* Resume */
		rc = dhdpcie_pci_suspend_resume(bus->dev, state);
		bus->dhd->busstate = DHD_BUS_DATA;
		DHD_ERROR(("%s: Resume completed\n", __FUNCTION__));
...
dhd_bus_flow_ring_create_request(dhd_bus_t *bus, void *arg)
{
	...
	/* Send Msg to device about flow ring creation */
	status = dhd_prot_flow_ring_create(bus->dhd, flow_ring_node);
	if (status != BCME_OK)
		return status;
